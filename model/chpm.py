...
class CHPMModel(nn.Module):
    ...
    def configure_student_layer_recall_fp32_island(self) -> Tuple[str, ...]:
        ...
        if bool(getattr(self.layer_recall_config, "layer_recall_dynamic_layer_enabled", False)):
            expected_dynamic = {
                "layer_recall_dynamic_state_norm",
                "layer_recall_dynamic_router",
            }
            missing_dynamic = sorted(expected_dynamic.difference(installed))
            if missing_dynamic:
                raise RuntimeError(
                    "Dynamic LayerRecall FP32 island is incomplete: "
                    f"missing {missing_dynamic}"
                )
    ...
    def layer_recall_architecture_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dynamic_layer_enabled": bool(
                getattr(config, "layer_recall_dynamic_layer_enabled", False)
            ),
            "dynamic_layer_budget": int(
                getattr(config, "layer_recall_dynamic_layer_budget", len(enabled_layer_ids))
            ),
