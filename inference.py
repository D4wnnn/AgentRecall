...
def _is_dynamic_layer_router_key(key):
    return any(token in str(key) for token in (
        "layer_recall_dynamic_state_norm",
        "layer_recall_dynamic_router",
        "layer_recall_dynamic_layer_embedding",
        "layer_recall_dynamic_layer_prior_logits",
    ))


def _preflight_layer_recall_state_dict(
    model_state,
    layer_recall_state,
    *,
    allow_missing_dynamic_layer_router=False,
):
    ...
    if allow_missing_dynamic_layer_router:
        missing_in_checkpoint = [
            key for key in missing_in_checkpoint if not _is_dynamic_layer_router_key(key)
        ]
    shape_mismatch = [
...
def _load_layer_recall_checkpoint_if_configured(pipeline, config, local_rank):
    ...
    dynamic_layer_enabled = _config_bool(
        _nested_get(
            config,
            "layer_recall",
            "layer_recall_dynamic_layer_enabled",
            default=False,
        )
    )
    _preflight_layer_recall_state_dict(
        model_state,
        layer_recall_state,
        allow_missing_dynamic_layer_router=dynamic_layer_enabled,
    )
    ...
    if dynamic_layer_enabled:
        missing_layer_recall = [
            key for key in missing_layer_recall if not _is_dynamic_layer_router_key(key)
        ]
