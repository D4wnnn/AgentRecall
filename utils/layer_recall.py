...
class LayerRecallConfig:
    ...
    layer_recall_dynamic_layer_enabled: bool = False
    layer_recall_dynamic_layer_budget: int = 10
    layer_recall_dynamic_layer_temperature: float = 1.0
    layer_recall_dynamic_layer_embed_dim: int = 64
    layer_recall_dynamic_layer_hidden_dim: int = 256
    layer_recall_dynamic_layer_prior_scale: float = 4.0
    layer_recall_score_mode: str = "cosine"
    layer_recall_physical_cache_frames: int = 0
    layer_recall_retrieval_policy: str = "cosine"
    layer_recall_agent_plan: Tuple[Tuple[int, int, Tuple[int, ...]], ...] = ()
    layer_recall_agent_plan_path: str = ""
    layer_recall_diversity_lambda: float = 0.0
    ...
    def validate(self, num_layers: Optional[int] = None) -> None:
        ...
        if self.layer_recall_retrieval_policy not in {
            "cosine", "recent", "oldest", "random", "agent_plan", "agent_event",
            "agent_event_diverse",
        }:
            raise ValueError(
                "layer_recall_retrieval_policy must be cosine, recent, oldest, "
                "random, agent_plan, agent_event, or agent_event_diverse, got "
                f"{self.layer_recall_retrieval_policy!r}"
            )
        parse_layer_ids(
            self.memory_sensitive_layers,
            field_name="memory_sensitive_layers",
        ...
        if bool(self.layer_recall_dynamic_layer_enabled) and not (
            1 <= int(self.layer_recall_dynamic_layer_budget) <= num_layers
        ):
            raise ValueError(
                "layer_recall_dynamic_layer_budget must be in [1, num_layers], "
                f"got {self.layer_recall_dynamic_layer_budget} for {num_layers} layers"
            )
        if float(self.layer_recall_dynamic_layer_temperature) <= 0:
            raise ValueError("layer_recall_dynamic_layer_temperature must be positive")
        if int(self.layer_recall_dynamic_layer_embed_dim) <= 0:
            raise ValueError("layer_recall_dynamic_layer_embed_dim must be positive")
        if int(self.layer_recall_dynamic_layer_hidden_dim) <= 0:
            raise ValueError("layer_recall_dynamic_layer_hidden_dim must be positive")
    ...
    def from_repo_config(cls, repo_config: Any, YX_rank: int = 0) -> "LayerRecallConfig":
        ...
        plan_path = str(get_config_value(raw, "layer_recall_agent_plan_path", "") or "")
        raw_plan = get_config_value(raw, "layer_recall_agent_plan", ()) or ()
        if plan_path:
            with open(os.path.expandvars(plan_path), "r", encoding="utf-8") as handle:
                plan_payload = json.load(handle)
            raw_plan = (
                plan_payload.get("layer_recall_agent_plan", plan_payload)
                if isinstance(plan_payload, dict)
                else plan_payload
            )
        parsed_plan = []
        for entry in raw_plan:
            current = get_config_value(entry, "current_chunks", ())
            preferred = get_config_value(entry, "preferred_chunks", ())
            if len(current) != 2:
                raise ValueError("agent plan current_chunks must be [start, end]")
            parsed_plan.append((
                int(current[0]), int(current[1]), tuple(int(item) for item in preferred)
            ))
        return cls(
            ...
            layer_recall_dynamic_layer_enabled=bool(get_config_value(
                raw, "layer_recall_dynamic_layer_enabled", False
            )),
            layer_recall_dynamic_layer_budget=int(get_config_value(
                raw, "layer_recall_dynamic_layer_budget", 10
            )),
            layer_recall_dynamic_layer_temperature=float(get_config_value(
                raw, "layer_recall_dynamic_layer_temperature", 1.0
            )),
            layer_recall_dynamic_layer_embed_dim=int(get_config_value(
                raw, "layer_recall_dynamic_layer_embed_dim", 64
            )),
            layer_recall_dynamic_layer_hidden_dim=int(get_config_value(
                raw, "layer_recall_dynamic_layer_hidden_dim", 256
            )),
            layer_recall_dynamic_layer_prior_scale=float(get_config_value(
                raw, "layer_recall_dynamic_layer_prior_scale", 4.0
            )),
            ...
            layer_recall_retrieval_policy=str(
                get_config_value(raw, "layer_recall_retrieval_policy", "cosine")
            ).lower(),
            layer_recall_agent_plan=tuple(parsed_plan),
            layer_recall_agent_plan_path=plan_path,
            layer_recall_diversity_lambda=max(
                0.0, float(get_config_value(raw, "layer_recall_diversity_lambda", 0.0))
            ),
...
def apply_retrieval_policy(
    ...
    *,
    ...
    layer_index: int,
    agent_plan: Sequence[Tuple[int, int, Sequence[int]]] = (),
    diversity_lambda: float = 0.0,
) -> Tuple[torch.Tensor, List[int]]:
    """Apply an inference-time memory policy without changing the K/V budget."""
    policy = str(policy).lower()
    if policy == "cosine" or scores.numel() == 0:
        return scores.float(), []
    chunk_ids = [int(record.chunk_index) for record in records]
    if policy == "recent":
        return torch.tensor(chunk_ids, device=scores.device, dtype=torch.float32), []
    if policy == "oldest":
        return -torch.tensor(chunk_ids, device=scores.device, dtype=torch.float32), []
    if policy == "random":
        values = [
            (((chunk_id + 1) * 1103515245
              + (int(current_chunk_index) + 1) * 12345
              + (int(layer_index) + 1) * 2654435761)
             & 0x7fffffff) / float(0x7fffffff)
            for chunk_id in chunk_ids
        ]
        return torch.tensor(values, device=scores.device, dtype=torch.float32), []
    if policy in {"agent_plan", "agent_event", "agent_event_diverse"}:
        preferred = []
        matched_plan = False
        for start, end, planned_chunks in agent_plan:
            if int(start) <= int(current_chunk_index) <= int(end):
                matched_plan = True
                preferred = [int(item) for item in planned_chunks]
                break
        if policy in {"agent_event", "agent_event_diverse"}:
            # An explicit empty event plan is a typed abstention. Falling back to cosine here
            # can leak a departed subject into a shot that deliberately contains no subject.
            if matched_plan and not preferred:
                return scores.new_empty((0,), dtype=torch.float32), []
            preferred_set = set(preferred)
            bonuses = torch.tensor(
                [10.0 if chunk_id in preferred_set else 0.0 for chunk_id in chunk_ids],
                device=scores.device,
                dtype=torch.float32,
            )
            routed = scores.float() + bonuses
            if policy == "agent_event_diverse" and float(diversity_lambda) > 0:
                allowed = [idx for idx, chunk_id in enumerate(chunk_ids) if chunk_id in preferred_set]
                if len(allowed) >= 2:
                    allowed_tensor = torch.tensor(allowed, device=scores.device, dtype=torch.long)
                    top_local = int(torch.argmax(scores.float()[allowed_tensor]).item())
                    top_index = int(allowed[top_local])
                    top_summary = F.normalize(
                        records[top_index].summary.detach().float().flatten(), dim=0
                    )
                    similarities = []
                    for record in records:
                        summary = F.normalize(record.summary.detach().float().flatten(), dim=0)
                        similarities.append(torch.dot(summary, top_summary))
                    penalty = torch.stack(similarities).to(device=scores.device)
                    routed = routed - float(diversity_lambda) * penalty
                    routed[top_index] = scores.float()[top_index] + bonuses[top_index] + 1.0
            return routed, preferred
        rank = {chunk_id: len(preferred) - pos for pos, chunk_id in enumerate(preferred)}
        bonuses = torch.tensor(
            [10.0 * rank.get(chunk_id, 0) for chunk_id in chunk_ids],
            device=scores.device,
            dtype=torch.float32,
        )
        return scores.float() + bonuses, preferred
...
def budgeted_straight_through_layer_gate(
    logits: torch.Tensor,
    *,
    budget: int,
    ...
    """Return an exact Top-r forward gate with sigmoid surrogate gradients.

    The hard gate enforces a fixed layer-compute budget.  During training its
    numerical forward value remains binary while gradients flow through the
    soft probabilities.  This keeps comparisons to fixed-layer LayerRecall
    matched in both active-layer count and attention-visible token count.
    """
    if logits.ndim != 1:
        raise ValueError(f"layer gate logits must be one-dimensional, got {tuple(logits.shape)}")
    num_layers = int(logits.numel())
    if not 1 <= int(budget) <= num_layers:
        raise ValueError(f"budget must be in [1, {num_layers}], got {budget}")
    if float(temperature) <= 0:
        raise ValueError(f"temperature must be positive, got {temperature}")
    probabilities = torch.sigmoid(logits / float(temperature))
    hard_gate = torch.zeros_like(probabilities)
    selected = torch.topk(logits.detach(), k=int(budget), largest=True).indices
    hard_gate.scatter_(0, selected, 1.0)
    if not training:
        return hard_gate, probabilities
    return straight_through_hard_value(hard_gate, probabilities), probabilities
