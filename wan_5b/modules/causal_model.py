...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        layer_recall_dynamic_layer_gate=None,
    ):
        r"""
        Args:
        ...
        if kv_cache is None:
            ...
            if layer_recall_enabled:
                if bool(getattr(layer_recall_config, "layer_recall_dynamic_layer_enabled", False)):
                    YX_memory_sensitive_layer = True
                    YX_layer_role = "dynamic_layer_candidate"
                else:
            ...
            if (
                ...
                if YX_gate_active:
                    ...
                    YX_retrieval_policy = str(
                        getattr(layer_recall_config, "layer_recall_retrieval_policy", "cosine")
                    ).lower()
                    YX_current_chunk_index = int(layer_recall_context.get("YX_chunk_index", 0))
                    YX_candidate_scores, YX_agent_preferred_chunk_ids = apply_retrieval_policy(
                        YX_candidate_scores,
                        YX_candidate_records,
                        policy=YX_retrieval_policy,
                        current_chunk_index=YX_current_chunk_index,
                        layer_index=int(layer_recall_layer_index),
                        agent_plan=getattr(layer_recall_config, "layer_recall_agent_plan", ()),
                        diversity_lambda=float(
                            getattr(layer_recall_config, "layer_recall_diversity_lambda", 0.0)
                        ),
                    )
                    ...
                    if YX_plan:
                        ...
                        if YX_memory_slots_filled > 0:
                            ...
                            for YX_slot_index, selected_record in enumerate(selected_records_for_plan):
                                ...
                                routed_k = materialize_layer_recall_slot(
                                    hard_k, soft_k, YX_selection_mode
                                )
                                routed_v = materialize_layer_recall_slot(
                                    hard_v, soft_v, YX_selection_mode
                                )
                                if layer_recall_dynamic_layer_gate is not None:
                                    # The inactive branch is the exact local-window
                                    # slot that this remote slot replaces.  Since the
                                    # straight-through gate is binary in the forward
                                    # pass, inactive layers reproduce local attention
                                    # instead of attending to artificial zero K/V.
                                    recent_start = (
                                        int(local_start_index)
                                        - int(YX_memory_slots_filled - YX_slot_index)
                                        * int(num_new_tokens)
                                    )
                                    recent_end = recent_start + int(num_new_tokens)
                                    if recent_start >= int(YX_base_sink_tokens):
                                        recent_k, recent_v = YX_get_logical_kv_range(
                                            recent_start, recent_end
                                        )
                                        dynamic_gate = layer_recall_dynamic_layer_gate.to(
                                            device=routed_k.device, dtype=routed_k.dtype
                                        ).view(1, 1, 1, 1)
                                        routed_k = recent_k + dynamic_gate * (routed_k - recent_k)
                                        routed_v = recent_v + dynamic_gate * (routed_v - recent_v)
                                slot_k_parts.append(routed_k)
                                slot_v_parts.append(routed_v)
                ...
                if layer_recall_logger is not None:
                    ...
                    YX_selection_payload = {
                        ...
                        **YX_current_info,
                        ...
                        "layer_recall_dynamic_layer_enabled": bool(
                            getattr(layer_recall_config, "layer_recall_dynamic_layer_enabled", False)
                        ),
                        "layer_recall_dynamic_layer_selected": bool(
                            layer_recall_dynamic_layer_gate is not None
                            and float(layer_recall_dynamic_layer_gate.detach().float().item()) > 0.5
                        ),
                        ...
                        "YX_agent_preferred_chunk_ids": [
                            int(item) for item in YX_agent_preferred_chunk_ids
                        ],
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        YX_dynamic_gate_for_layer = None
        if (
            layer_recall_dynamic_layer_gate is not None
            and 0 <= int(layer_recall_layer_index) < int(layer_recall_dynamic_layer_gate.numel())
        ):
            YX_dynamic_gate_for_layer = layer_recall_dynamic_layer_gate[
                int(layer_recall_layer_index)
            ]
            layer_recall_current_info["layer_recall_dynamic_layer_gate"] = float(
                YX_dynamic_gate_for_layer.detach().float().item()
            )
        self_attn_result = self.self_attn(
            ...
            layer_recall_dynamic_layer_gate=YX_dynamic_gate_for_layer)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.layer_recall_dynamic_state_norm = None
        self.layer_recall_dynamic_router = None
        self.register_parameter("layer_recall_dynamic_layer_embedding", None)
        self.register_parameter("layer_recall_dynamic_layer_prior_logits", None)
        self.layer_recall_last_dynamic_gate = None
        self.layer_recall_last_dynamic_gate_probs = None
    ...
    def configure_layer_recall(self, layer_recall_config: LayerRecallConfig) -> None:
        ...
        if bool(layer_recall_config.layer_recall_enabled) and bool(
            getattr(layer_recall_config, "layer_recall_dynamic_layer_enabled", False)
        ):
            embed_dim = int(layer_recall_config.layer_recall_dynamic_layer_embed_dim)
            hidden_dim = int(layer_recall_config.layer_recall_dynamic_layer_hidden_dim)
            if self.layer_recall_dynamic_state_norm is None:
                self.layer_recall_dynamic_state_norm = nn.LayerNorm(self.dim)
            if self.layer_recall_dynamic_layer_embedding is None:
                self.layer_recall_dynamic_layer_embedding = nn.Parameter(
                    torch.randn(len(self.blocks), embed_dim, dtype=torch.float32) * 0.02
                )
            if self.layer_recall_dynamic_router is None:
                self.layer_recall_dynamic_router = nn.Sequential(
                    nn.Linear(self.dim + embed_dim, hidden_dim),
                    nn.SiLU(),
                    nn.Linear(hidden_dim, 1),
                )
                nn.init.zeros_(self.layer_recall_dynamic_router[-1].weight)
                nn.init.zeros_(self.layer_recall_dynamic_router[-1].bias)
            if self.layer_recall_dynamic_layer_prior_logits is None:
                prior_scale = float(layer_recall_config.layer_recall_dynamic_layer_prior_scale)
                prior = torch.full((len(self.blocks),), -prior_scale, dtype=torch.float32)
                prior[list(layer_recall_config.memory_sensitive_layers)] = prior_scale
                self.layer_recall_dynamic_layer_prior_logits = nn.Parameter(prior)
    ...
    def _forward_inference(
        ...
        # Predict all layer decisions once from the current noisy chunk.  The
        # same state is paired with learned layer embeddings and a shared MLP,
        # so this is a state-conditioned router rather than 30 independent
        # classifiers.  Its prior exactly reproduces the released fixed layer
        # set before training.
        layer_recall_dynamic_layer_gate = None
        self.layer_recall_last_dynamic_gate = None
        self.layer_recall_last_dynamic_gate_probs = None
        if (
            bool(getattr(self.layer_recall_config, "layer_recall_enabled", False))
            and bool(getattr(self.layer_recall_config, "layer_recall_dynamic_layer_enabled", False))
            and self.layer_recall_dynamic_state_norm is not None
            and self.layer_recall_dynamic_router is not None
            and self.layer_recall_dynamic_layer_embedding is not None
            and self.layer_recall_dynamic_layer_prior_logits is not None
        ):
            router_dtype = x.dtype
            state_summary = current_token_summary(
                x,
                token_dim=1,
                detach=bool(
                    getattr(
            ...
            state_summary = _layer_recall_module_forward_in_dtype(
                self.layer_recall_dynamic_state_norm,
                state_summary,
                router_dtype,
            )
            num_layers = len(self.blocks)
            state_by_layer = state_summary.unsqueeze(0).expand(num_layers, -1)
            layer_embedding = self.layer_recall_dynamic_layer_embedding.to(
                device=x.device, dtype=router_dtype
            )
            router_input = torch.cat([state_by_layer, layer_embedding], dim=-1)
            router_delta = _layer_recall_module_forward_in_dtype(
                self.layer_recall_dynamic_router,
                router_input,
                router_dtype,
            ).squeeze(-1)
            router_logits = (
                self.layer_recall_dynamic_layer_prior_logits.to(
                    device=x.device, dtype=router_dtype
                )
                + router_delta
            )
            layer_recall_dynamic_layer_gate, layer_gate_probs = (
                budgeted_straight_through_layer_gate(
                    router_logits,
                    budget=int(self.layer_recall_config.layer_recall_dynamic_layer_budget),
                    temperature=float(
                        self.layer_recall_config.layer_recall_dynamic_layer_temperature
                    ),
                    training=bool(torch.is_grad_enabled()),
                )
            )
            self.layer_recall_last_dynamic_gate = layer_recall_dynamic_layer_gate
            self.layer_recall_last_dynamic_gate_probs = layer_gate_probs
        ...
        kwargs = dict(
            ...
            layer_recall_dynamic_layer_gate=layer_recall_dynamic_layer_gate,
        )
